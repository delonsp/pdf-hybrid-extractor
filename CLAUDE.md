...
- Time budget (Lote B — **the numbers are calibration, tune them in Dokploy without a deploy**):
  ...
  - `MAX_CONCURRENT_EXTRACTIONS` (6) and `GUNICORN_THREADS` (8) — **invariant: the semaphore must stay strictly below the thread count.** The slack is what keeps `/health` answering and lets an over-capacity request get a fast 503; raising only the semaphore pushes the excess into the backlog and kills both guarantees. `create_app()` logs an ERROR if the invariant is violated. Over capacity returns **503 + `Retry-After`** instead of queueing: waiting comes out of the same caller budget, so a queue only converts fast rejection into slow, silent timeout.
  - Concurrent Gemini calls peak at `MAX_CONCURRENT_EXTRACTIONS × VISION_PARALLEL` — 18 at the current defaults. Measured RSS (5-page scan-like PDF, per-process peak): 3×3 → 310 MB, 6×3 → 460 MB, 6×2 → 392 MB. Doubling concurrency costs ~150 MB rather than doubling, because `_pymupdf_lock` serialises rendering process-wide so only one pixmap exists at a time. **If memory gets tight, lower `VISION_PARALLEL` rather than the slot count** — it trades one document's latency for batch throughput, which is the right trade for a burst. Note the raw-bytes worst case is separate: `MAX_CONCURRENT_EXTRACTIONS × MAX_DOWNLOAD_BYTES` = 300 MB of PDF bytes alone at the defaults.
  - `TRUST_REQUEST_START` (`true`) — if the caller sends `X-Request-Start` (epoch seconds or ms, `t=` prefix tolerated), time spent waiting in the backlog is **subtracted from the deadline**. Without it, `REQUEST_DEADLINE` starts when the handler runs, not when the request arrived: a request that queued for 60s would blow the caller's 120s while this service still considered itself on time. Implausible values (future, or beyond `MAX_PLAUSIBLE_QUEUE_WAIT`) are ignored — clocks on different machines are not trustworthy. If no useful budget remains, the request gets a fast 503 instead of guaranteed-orphan work.
...
**`vision_diagnostics` / `pages_output_truncated`** carry per-page reasons for an empty or cut-off Vision response. Empty used to be logged uniformly as "probably a safety filter", but the causes need different reactions: `vazio:safety` (retrying won't help), `vazio:recitation`, `prompt_bloqueado:*`, and `truncado_max_tokens`. **Truncation is the dangerous one** — the model stops mid-output and the text comes back looking complete, so a lab value can be cut in half. It flips `complete` to `false`. `VISION_MAX_OUTPUT_TOKENS` (8192) is set explicitly rather than inherited from the model's default, which shifts when the alias moves. A page rescued by the fallback model has its diagnostic cleared — it worked, so it must not show up as a problem.
