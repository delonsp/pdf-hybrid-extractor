...
- [x] B9. Minio: validar antes de gravar; falha vira erro explícito (`minio_stored` / `minio_error`); corrida do bucket resolvida; mecanismo de chave pseudonimizada pronto
- [ ] B9b. **Rollout da pseudonimização** (`MINIO_KEY_MODE=pseudonimo` + `MINIO_KEY_SALT`): default segue em `telefone` porque virar pseudônimo muda o layout do bucket e **quebra busca por prefixo nos objetos já gravados**. Decidir: migrar o histórico, aceitar layout misto, ou manter. Enquanto não virar, o telefone segue exposto em listagem, métrica e no `minio_path` da resposta
- [x] B9c. **RETENÇÃO — decidido e implementado (06/08/2026).** Prazo: **60 dias**. Razão do Dr. Alain: o bucket é área de trabalho para interpretação, **não repositório eterno de exames**. Vira regra de lifecycle no bucket (`MINIO_RETENTION_DAYS=60`, aplicada uma vez por processo); quem apaga é o Minio. Falha ao aplicar não derruba o upload, mas também não é marcada como feita — senão a retenção sumiria em silêncio até o próximo restart
- [ ] B9d. **ANTES DE DEPLOYAR:** a regra vale para o bucket INTEIRO, incluindo o que já está lá. O serviço roda desde fev/2026, então há objetos bem mais velhos que 60 dias — todos serão apagados na primeira varredura do Minio, e isso **não tem volta**. Conferir o que existe hoje e fazer cópia do que for necessário guardar antes de subir
